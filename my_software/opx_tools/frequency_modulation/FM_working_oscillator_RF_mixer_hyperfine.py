...
from my_software.opx_tools.frequency_modulation.configuration_FM_working_oscillator_RF_mixer_hyperfine import \
    parameters, IQ_imbalance
import pandas as pd
...
class FM_setup:
    ...
    def __init__(self, f_mod=7e3, f_dev=250e3, OPX_LO_voltage=0.01, OPX_IF_voltage=0.5, g_cor=0.11474, phi_cor=-0.09787,
                 I_offset=-0.01040,
                 Q_offset=-0.01000):
    ...
    def execute_FM(self):
        ...
        for machine in qmm.list_open_quantum_machines():
            ...
            try:
                ...
                continue
            if self.config_parameters.port_I in ports or self.config_parameters.port_Q in ports or self.config_parameters.port_IF_hyperfine in ports:
        ...
        with program() as prog:
            wait(44, "LIA")
            frame_rotation_2pi(-0.25, "LIA")
            #update_frequency("ensemble", self.config_parameters.f_base)
            reset_phase("LIA")

            align("ensemble", "LIA")

            play("const", "ensemble", chirp=(self.rates, self.units), continue_chirp=False)
            ...
            with infinite_loop_():
                ...
                #align("ensemble", "LIA")
                play("zero", "ensemble", chirp=(self.rates, self.units), continue_chirp=False)
                play("const_single", "LIA")
                reset_frame("LIA")
...
if __name__ == '__main__':
    ...
    fm = FM_setup(OPX_LO_voltage=OPX_LO_voltage, OPX_IF_voltage=OPX_IF_voltage, f_dev=f_dev, f_mod=f_mod, g_cor=g,
                  phi_cor=phi, I_offset=I, Q_offset=Q, )
    fm.execute_FM()
