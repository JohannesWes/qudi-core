...
DEFAULT_SAMPLING_RATE = 53570  # Hz
DEFAULT_TIMEOUT = 180  # seconds, maximum wait for acquisition to finish
...
class LockInAmp:
    ...
    def __init__(self, server_host=DEFAULT_SERVER_HOST, device_id=DEFAULT_DEVICE_ID):
        """
        Initialize a LockInAmp instance.

        Args:
            server_host (str): The IP or hostname of the Zurich Instruments server.
    ...
    def demod_data_acquisition(self, total_duration, n_samples, timeout=DEFAULT_TIMEOUT):
        ...
        logger.info(f"Data acquisition started for {total_duration} seconds.")

        # --- THE CORRECTED FIX: Actively poll for progress and completion ---
        start_time = time.time()
        while not self.daq_module.finished():
            ...
            # Get and log the progress
            progress = self.daq_module.progress()
            #logger.info(f"Progress: {progress[0] * 100:.1f}%")

            # Wait for a short interval before polling again to avoid flooding the log
            time.sleep(1.0)  # Poll every 1 second

        logger.info("DAQ module has finished.")

        # Now that the loop is done, read the data. This will be non-blocking.
        result = self.daq_module.read(raw=False, clk_rate=self.device.clockbase())
        logger.info("Results read from DAQ module.")

        # Extract time, x, and y data
        if not result or demod_sample_nodes[0] not in result:
            raise RuntimeError(f"Failed to retrieve data for node {demod_sample_nodes[0]}. "
                               f"Result dictionary is empty or incomplete.")
...
class SensitivityMeasurement:
    """
    Class that uses a LockInAmp instance to perform a sensitivity measurement.

    Responsibilities:
        - High-level measurement routines that involve multiple data acquisitions
          and result processing steps.
        - Optionally save or plot the acquired data.

    This class does not handle low-level device communication directly.
    """

    def __init__(self, lock_in_amp: LockInAmp):
        """
        Initialize with an existing LockInAmp instance.

        Args:
            lock_in_amp (LockInAmp): An instance of the LockInAmp class.
        """
        self.lia = lock_in_amp
    ...
    def run_measurement(self,
        ...
        # Acquire data
        acq_data = self.lia.demod_data_acquisition(total_duration=total_duration,
                                                   n_samples=n_samples,
                                                   timeout=timeout)
        ...
        if plot_data:
            ...
            ax.plot(times, x_value, label="X-Channel [V]")
            #ax.plot(times, y_value, label="Y-Channel [V]", alpha=0.7)
            ax.grid(True)
            ax.legend()
            ax.set_title("Data acquired through the DAQ module")
...
if __name__ == "__main__":
    ...
    LIA = LockInAmp()
    sm = SensitivityMeasurement(LIA)
    sm.run_measurement("test",
                       n_time_traces=5,
                       save_raw_data=True,
                       plot_data=True)
