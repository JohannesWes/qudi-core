    ...
    for peak_index, dip_index in zip(peaks_indices, dips_indices):
        ...
        try:
            if plot_fitting:
                ...
                ax.scatter(frequencies_around_peak, voltages_around_peak, color='blue', alpha=0.3)
                ax.scatter(frequencies_around_dip, voltages_around_dip, color='blue', alpha=0.3, label='data')
                ax.plot(frequencies_around_dip, smooth(voltages_around_dip, 20), zorder=3, color="yellow", linewidth=3)
                ax.plot(frequencies_around_peak, smooth(voltages_around_peak, 20), zorder=3, color="yellow", label="smoothed data", linewidth=3)
                ax.plot(frequencies_around_peak, parabola(frequencies_around_peak, *peak_params), color='red', zorder=2, linewidth=2)
                ax.plot(frequencies_around_dip, parabola(frequencies_around_dip, *dip_params), color='red', label="fit", zorder=2, linewidth=2)
                ax.scatter(peak_position, parabola(peak_position, *peak_params), color='green', zorder=3)
                ax.scatter(dip_position, parabola(dip_position, *dip_params), color='green', zorder=3)
                ax.legend()
                fig.show()
                plt.show()
