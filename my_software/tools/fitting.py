...
def fit_hyperfine(frequency_array, voltage_array, feature_distance_in_Hz=0.5e6, min_feature_amplitude=0.02,
                  min_feature_height=0.02, use_peak_finding_if_fitting_fails=True, n_most_prominent_peaks, zero_crossings_fit_range=0.1e6, feature_fit_range=0.2e6, plot_fitting=False):
    """
    This function finds the peaks and dips in the ODMR signal and fits straight lines around the zero crossings.
    It is ment for high-quality hyperfine ODMR spectra, where the peaks/dips can be identified by the maximum/minimum
    ...
    # Fit straight lines around the zero crossings
    for zero_crossing_f in zero_crossings_frequencies:
        # Define the range for fitting
        f_fit_start = zero_crossing_f - zero_crossings_fit_range
        f_fit_end = zero_crossing_f + zero_crossings_fit_range
        index_fit_start, index_fit_end = np.argmin(np.abs(frequency_array - f_fit_start)), np.argmin(np.abs(frequency_array - f_fit_end))
