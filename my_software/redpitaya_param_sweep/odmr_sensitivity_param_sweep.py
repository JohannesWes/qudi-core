    ...
    for idx, combination in enumerate(product_params):
        ...
        try:
            ...
            filename_prefix = os.path.join(folder_name,
                                           f"P_{power_val:.2f}dBm_fmod_{f_mod_val / 1e3:.1f}k_fdev_{f_dev_val:.1f}k")
            ...
            if include_off_resonant_sensitivity:
                off_resonant_cw_frequency = cw_frequency + 20e6  # 20 MHz offset
            ...
            # <<< --- THIS WAS THE MISSING PART (ADDED BACK) --- >>>
...
if __name__ == "__main__":
    ...
    # Total power of the microwave signal in dBm.
    odmr_power_dbm_array = np.linspace(-12.5, -10, 1)

    # FM modulation frequency in Hz.
    fm_modulation_frequency_array = np.linspace(14e3, 25e3, 1)
    ...
    laser_power_mW = 1000 * (0.11/0.5)  # For metadata
